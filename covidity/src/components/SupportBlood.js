...
function SupportBlood({countryStates,districts,getCountryStates,getDistricts,getOxygenDonors,oxygenDonors}) {
    ...
    const [selectedDistrict1,setSelectedDistrict1]=useState("none")
    ...
    const [selectedState1,setSelectedState1]=useState("none")
    ...
    return (
      ...
      {({ inputProps, focused }) => (
        ...
        {( useremail.length>0 && !validator.validate(useremail)) || !phno|| phno.length<10 ||(!userAge || userAge<0 || userAge>120) ||!fullName || (exaddress=="" || (patientSpo2>100||patientSpo2<0))?(<><span style={{fontFamily:"Krona One, sans-serif",fontSize:"11px", color:"white", paddingTop:"10px", marginLeft:"30%"}}>* Fill all the required fields</span></>):(<div style={{display:"flex", flexWrap:"wrap", alignItems:"center", justifyContent:"center"}}>
            ...
            <Dropdown>
                    <Dropdown.Toggle className="btn2" variant="success" id="dropdown-basic">
                        {selectedState1=="none"? (<>Select State</>): (<>{selectedState1}</>)} 
                      </Dropdown.Toggle>

                      <Dropdown.Menu>
                         {countryStates && countryStates.countryStates && countryStates.countryStates.map((elem)=> <Dropdown.Item onClick={()=>{setSelectedState1(elem["state_name"]); setState_id(elem["state_id"]); getDistricts(elem["state_id"])}}>{elem["state_name"]}</Dropdown.Item>)}
                      </Dropdown.Menu>
                    </Dropdown>
                    ...
                    {selectedState1=="none" ? (<></>) : (
                      <Dropdown>
                        <Dropdown.Toggle className="btn2" variant="success" id="dropdown-basic">
                         {selectedDistrict1=="none"?(<>Select District</>) : ( <>{selectedDistrict1}</>)}
                        </Dropdown.Toggle>

                        <Dropdown.Menu>
                            {districts && districts.districts && districts.districts.map((elem)=> <Dropdown.Item onClick={()=>{setDist_id(elem["district_id"]); setSelectedDistrict1(elem["district_name"]);  getOxygenDonors(elem["district_id"]); setDispCompOpen(!oxygenDonors?"none":"flex");}}>{elem["district_name"]}</Dropdown.Item>)}
                        </Dropdown.Menu>
                      </Dropdown>
                    )}
